...
module.exports = { nodes };`;
    fs.writeFileSync('distribution/engine/nodes.js', nodesContent);

    // Run the test and capture output
    const output = execSync(`node test/test-student/m6.indexer.test.js`, { encoding: 'utf-8' });
...
async function generateCharts(results) {
  ...
  fs.writeFileSync(path.join(outputDir, 'latency-chart.png'), latencyImage);
  fs.writeFileSync(path.join(outputDir, 'throughput-chart.png'), throughputImage);
  
  console.log(`Charts saved to ${outputDir}`);
}

// Main test function
async function runVisualizedTest() {
...
runVisualizedTest().catch(console.error);
